---
title: v2.4.0
date: 2025-06-31
---

Although a minor release, this is a **major release** in terms of features!

**🚀 Features**

- **Structure-aware blocks generation** that supports every project's block configuration and generates content based on your unique block definitions. See the [Blocks & Layouts](/docs/copilot/usage/blocks-and-layouts) documentation.
- New content generation for [`layout` fields](/docs/copilot/usage/blocks-and-layouts).
- Support for Google Gemini models.

:prose-video{src="/vid/kirby-copilot-generate-layouts.mp4" poster="/vid/kirby-copilot-generate-layouts-poster.jpg" width="908" height="540"}
