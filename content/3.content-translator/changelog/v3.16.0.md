...
- **A batch report per language**: When a language was not translated or not saved, the run ends in a dialog that names it with the reason, such as DeepL's exhausted quota. Batch translation now needs Kirby 5. See [Translation Results](/docs/content-translator/panel/translation-results).
...
- **Slugs follow the target language**: A translated slug follows the slug rules of the target language, so a Turkish `Ürünler` becomes `urunler`, and a Chinese title gets a slug at all.
...
- **A saved language is reported as saved**: A language whose content was saved is no longer reported as failed.
- **Code blocks stay as written**: Kirby's `code` block is no longer translated.
- **Copilot only when it can translate**: AI translation is offered only when Copilot is v3.9.0 or later and has an API key.
