---
title: v3.0.0
date: 2024-12-28
---

Kirby Content Translator v3 is a **major release** with full support for Kirby 5. This major release requires a new license key. If you already have a license, you receive a 50% discount on your new license. Head over to the [Kirby Tools Hub](https://hub.kirby.tools) to get your discount or read more in the [license compatibility guide](https://kirby.tools/license-compatibility).

**🚀 Features**

- **Panel view button**: [Panel view button](/docs/content-translator/configuration/local#view-button-configuration) as an alternative to the section. Including backward compatibility for Kirby 4 🎉.
- **Kirby 5**: Full compatibility with Kirby 5 including the new Panel architecture.
