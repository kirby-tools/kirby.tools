...
When the user clicks the Copilot button, the prompt dialog opens with the predefined prompt already filled in. The placeholder `{content}` will be replaced with the actual content of the writer field when the prompt is sent to the AI model.
