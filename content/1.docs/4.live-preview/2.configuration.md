...
::note
`updateInterval` throttles both strategies. Setting it to `false` only stops the `interval` strategy – with `updateStrategy: blur` the preview still rerenders on blur.
::
