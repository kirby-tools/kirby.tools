...
### `updateInterval` :u-badge{label="Number | Boolean" class="align-middle ml-2 rounded-full!" variant="subtle"}
...
Enabled by default – editors can click links and buttons inside the preview. Set it to `false` to make the preview inert – the page gets `pointer-events: none`, so clicks and hovers no longer reach it, and your own CSS cannot re-enable them.
...
The aspect ratio value must be a string formatted as `width/height` (e.g., `16/9`). Any other form, `16:9` included, throws when the section loads.
