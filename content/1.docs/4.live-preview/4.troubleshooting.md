...
The button carries no detail. Open the browser console for the actual error. Three causes come up: the page has no template file, the template renders no `<head>` tag, which the preview requires, and the template raised an error on the unsaved content it was handed.
