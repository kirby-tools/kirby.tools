...
To add the button to every site, page, or file view without editing each blueprint, set Kirby's `panel.viewButtons` option. It replaces the defaults the same way `buttons` in a blueprint does, and a blueprint that sets `buttons` wins over it:

```php [site/config/config.php]
return [
    'panel' => [
        'viewButtons' => [
            'site' => ['copilot', 'open', 'preview', 'languages'],
            'page' => ['copilot', 'open', 'preview', '-', 'settings', 'languages', 'status'],
            'file' => ['copilot', 'open', 'settings', 'languages']
        ]
    ]
];
```

The option takes button names only, so props such as `userPrompt` stay in the blueprint.
...
Custom system prompt that controls how the AI model structures and formats the generated content. Users cannot see or edit this prompt.
