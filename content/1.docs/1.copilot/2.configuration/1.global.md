...
::note
Google Gemini models are the recommended choice for [Blocks and Layouts](/docs/copilot/advanced/blocks-and-layouts) and other structured output: OpenAI's models cap the nesting depth those schemas need. Get a [Google API key](https://aistudio.google.com/api-keys) and set `google` as the provider.
::
...
Set it to `false` to stop ghost text from appearing on its own. The manual trigger, :kbd{value="meta"} :kbd{value=","}, keeps working:
...
### `promptTemplates` :u-badge{label="Array" class="align-middle ml-2 rounded-full!" variant="subtle"}
...
::callout{icon="i-ri-bookmark-line" color="info" to="/docs/copilot/prompt-dialog/templates#config-defined-templates"}
Learn more about prompt templates and how they appear in the Panel.
::

### `skills` :u-badge{label="Array" class="align-middle ml-2 rounded-full!" variant="subtle"} :u-badge{label="since v3.7.0" class="align-middle rounded-full" variant="subtle"}
...
### `api` :u-badge{label="String" class="align-middle ml-2 rounded-full!" variant="subtle"} :u-badge{label="since v3.6.0" class="align-middle mb-1 rounded-full" variant="subtle"}
...
| [Vercel AI Gateway](https://vercel.com/docs/ai-gateway) (`ai-gateway.vercel.sh/v1`)                                        | Yes           | —          |
| [Cloudflare AI Gateway – `…/openai`](https://developers.cloudflare.com/ai-gateway/usage/providers/openai/) (OpenAI only)   | Yes           | —          |
| [Cloudflare AI Gateway – `…/compat`](https://developers.cloudflare.com/ai-gateway/usage/chat-completion/) (many providers) | No            | **`chat`** |
| [OpenRouter](https://openrouter.ai/) (`openrouter.ai/api/v1`)                                                              | Yes           | —          |
...
::callout{icon="i-ri-external-link-line" color="info" to="https://developers.cloudflare.com/ai-gateway/usage/chat-completion/"}
...
### `timeout` :u-badge{label="Integer" class="align-middle ml-2 rounded-full!" variant="subtle"} :u-badge{label="since v3.12.0" class="align-middle mb-1 rounded-full" variant="subtle"}

Seconds a single request to the provider may take before it counts as failed. Configured per provider alongside `apiKey` and `model`. Applies to PHP runs through [`Client`](/docs/copilot/php-classes/client); Panel requests go through the proxy, which bounds them on its own. Anything but a positive integer falls back to the default.
...
### `options` :u-badge{label="Array" class="align-middle ml-2 rounded-full!" variant="subtle"}

A map the Panel passes to the AI SDK as this provider's options with every request, keyed by the option names the SDK defines for that provider. Configured per provider alongside `apiKey` and `model`. The PHP [`Client`](/docs/copilot/php-classes/client) ignores it; request parameters for PHP runs sit beside it as [provider-specific options](/docs/copilot/php-classes/providers/openai#provider-specific-options).
...
::callout{icon="i-ri-arrow-right-line" color="info" to="/docs/copilot/configuration/local#configuration-precedence"}
