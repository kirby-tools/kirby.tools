...
### `api` :u-badge{label="String" class="align-middle ml-2 rounded-full!" variant="subtle"} :u-badge{label="since v3.6.0" class="align-middle mb-1 ml-2 rounded-full" variant="subtle"}
...
::warning
**Structured output (blocks, layouts, field schemas) through OpenAI-compatible gateways depends on the gateway's `json_schema` translation.** `strict` mode and deeply nested schemas may fail when the upstream provider's compat shim doesn't fully implement the OpenAI JSON schema dialect. Test before relying on blocks or layout generation through this path.
::

::warning
**Authenticated gateways (`cf-aig-authorization`) are not currently supported.** Only the upstream provider's `Authorization: Bearer` key is forwarded. If your gateway requires a separate gateway-level token, please [open an issue](https://github.com/johannschopplich/kirby-copilot/issues).
::

::warning
**`reasoningEffort` over a cross-provider gateway prefix is suppressed.** When the configured `model` points at a non-OpenAI provider through the OpenAI-compat path (e.g. `google-ai-studio/gemini-3-flash-preview`), the OpenAI-shaped `reasoning_effort` field sent by the AI SDK doesn't reliably reach the downstream provider – Cloudflare's `/compat` forwards it as-is (most targets ignore or reject it), and Vercel's compat endpoint expects a different shape. For full reasoning control on Anthropic or Google models, use `provider: "anthropic"` or `provider: "google"` directly. If your gateway does honor OpenAI's `reasoning_effort`, force it through `providers.openai.options.reasoningEffort`.
::
