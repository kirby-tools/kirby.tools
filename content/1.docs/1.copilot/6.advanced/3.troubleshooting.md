...
With Kirby's `debug` option off, the Panel replaces an option value it cannot use with the default, drops a `promptTemplates` or `skills` entry it cannot read, and turns an unknown `provider` into Google, all without an error. With `debug` on, the Panel fails to load and the exception names the option, for example `Invalid reasoningEffort: max. Must be one of: provider-default, none, minimal, low, medium, high, xhigh`.
