...
description: Fix generation timeouts, API key errors, malformed blocks output, and missing inline suggestions in local and production setups.
---
...
All AI requests go through a server-side PHP proxy, so your web server has to keep the connection alive for the entire generation – which can take 60+ seconds for longer content. The culprit is usually a timeout at the web server level, not PHP itself.

::note
PHP's own execution limit is already lifted for proxy requests – the issue is typically nginx closing the connection before PHP finishes.
::
...
If the PHP `Client` fails with `Missing API key in "johannschopplich.copilot.providers.<name>.apiKey"`, or the Panel reports `Missing API key for the "<name>" provider`, the plugin received an empty API key for the selected provider:

1. Confirm the key is present under `providers.<name>.apiKey` – not at a higher level.
2. If you read the key from an environment variable, make sure the variable is actually set in the environment the Panel runs under. CLI and web server environments often differ.
...
The PHP `Client` fails with `Unknown provider "<name>" – set "johannschopplich.copilot.provider" to one of: openai, anthropic, google, mistral` when the top-level `provider` key holds a value that isn't a supported provider name (the comparison is case-insensitive), and with `Missing required option "johannschopplich.copilot.provider"` when the key is missing altogether.

::callout{icon="i-ri-arrow-right-line" color="info" to="/docs/copilot/php-classes/exceptions"}
See the **Exceptions** reference for all exception types and the structured detail payloads they carry.
::

## An Option Is Ignored

With Kirby's `debug` option off, the Panel replaces an option value it cannot use with the default, drops a `promptTemplates` or `skills` entry it cannot read, and turns an unknown `provider` into Google, all without an error. With `debug` on, Copilot's own surfaces fail with an exception that names the option while the rest of the Panel loads as usual, for example `Invalid reasoningEffort: max. Must be one of: provider-default, none, minimal, low, medium, high, xhigh`.
...
3. Raise the log level to `info` or `debug` and check the browser console for the system and user prompt that were actually sent:
...
1. Make sure the `copilot-suggestions` mark is added to your writer field. If the field already defines `marks`, append it to that list rather than replacing it – `marks` replaces Kirby's defaults, so every mark you want to keep has to stay in the list:
...
3. Behind an AI gateway with a prefixed `model`, set `completionModel` explicitly – Copilot won't derive one across gateways. See the [`completionModel`](/docs/copilot/configuration/global#default-models) option.
