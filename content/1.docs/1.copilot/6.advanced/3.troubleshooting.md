...
If requests fail with `Unknown provider "<name>" – set "johannschopplich.copilot.provider" to one of: openai, anthropic, google, mistral`, the top-level `provider` key in your config is missing or doesn't match a supported provider name (case-insensitive).
