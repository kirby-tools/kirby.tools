...
`Client` selects a provider via `match` on the resolved `ProviderName` enum. Each receives a `ProviderConfig` with `apiKey`, `model`, `baseUrl`, `timeout`, and an `options` bag forwarded to the upstream request. The keys the plugin reads itself – `completionModel`, `api`, `options`, and `timeout` – are dropped rather than forwarded.

## Retry Behavior

| Provider                        | Retry                                                                                              |
| ------------------------------- | -------------------------------------------------------------------------------------------------- |
| `OpenAIProvider` and subclasses | Built-in retry wrapper – see [details](/docs/copilot/php-classes/providers/openai#retry-behavior). |
| `AnthropicProvider`             | Delegated to the official `anthropic-ai/sdk`.                                                      |

Every provider applies the per-provider [`timeout`](/docs/copilot/configuration/global#timeout) to its HTTP client, so a stalled connection fails rather than blocking. A failed request throws `ProviderException` with the upstream error attached as `previous`. `OpenAIProvider` also passes the response body as `responseExcerpt`, and throws on the first attempt when the failure is not retriable – a 4xx status or an unreadable response.
