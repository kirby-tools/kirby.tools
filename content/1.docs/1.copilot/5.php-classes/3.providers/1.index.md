...
The `options` bag is whatever is left: every key under `providers.<name>` that is not one of those five ends up in the request payload as written. Vendor parameters therefore need no option of their own.
...
::warning
Panel generations are unaffected – they go through the proxy, which forwards the Panel's own request body. And since the bag reaches the API unchecked, a parameter the vendor rejects surfaces as a `ProviderException`.
::
