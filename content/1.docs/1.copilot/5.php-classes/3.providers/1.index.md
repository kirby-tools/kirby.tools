...
Every provider applies the per-provider [`timeout`](/docs/copilot/configuration/global#timeout) to its HTTP client, so a stalled connection fails rather than blocking. A failed request throws `ProviderException` with the upstream error attached as `previous`.
