...
A failed request throws `ProviderException` with the upstream error attached as `previous`. `OpenAIProvider` also passes the response body as `responseExcerpt`, and throws on the first attempt when the failure is not retriable – a 4xx status or an unreadable response.
