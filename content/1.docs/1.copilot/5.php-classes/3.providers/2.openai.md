...
The initial request plus up to 3 retries on `RateLimitException` (429), `ServerException` (5xx), `TransporterException` (network), and `ErrorException` with status 429 or ≥500. The wrapper honors `Retry-After` as seconds or as an HTTP date, clamped to between 1 and 60 seconds. Without a usable header it falls back to `2^attempt` seconds.
...
After the fourth failed request the wrapper throws `ProviderException` with `reason: 'request failed: <message>'`, `responseExcerpt` (body shortened to 200 chars), `httpCode`, and `previous` set to the original `Throwable`. A failure outside the list above – a 4xx other than 429, or an unreadable response – is not retried and throws on the first attempt.
...
`OpenAIProvider` always calls Chat Completions. The [`api` option](/docs/copilot/configuration/global#api) selects Chat Completions or the Responses API for the Panel only, and never reaches this class.
...
Anything in `providers.openai` that isn't `apiKey`, `model`, `baseUrl`, `completionModel`, `api`, `options`, or `timeout` lands in `ProviderConfig::$options` and is spread into every request payload:
