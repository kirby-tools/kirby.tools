...
Use OpenAI directly, or route through any OpenAI-compatible endpoint (OpenRouter, Ollama, Cloudflare AI Gateway, llama.cpp) by setting `baseUrl`. `OpenAIProvider` is also the parent class for [`GeminiProvider`](/docs/copilot/php-classes/providers/gemini) and [`MistralProvider`](/docs/copilot/php-classes/providers/mistral).
...
Up to 3 attempts on `RateLimitException` (429), `ServerException` (5xx), `TransporterException` (network), and `ErrorException` with status 429 or ≥500. The wrapper honours `Retry-After`, falling back to `2^attempt` seconds.
