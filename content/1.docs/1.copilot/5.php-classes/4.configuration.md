...
`fromKirbyOptions()` throws `InvalidArgumentException` on missing or unknown `provider`; `forProvider()` throws `AuthException` on missing API keys. See **Exceptions** for the full message catalogue.
::
