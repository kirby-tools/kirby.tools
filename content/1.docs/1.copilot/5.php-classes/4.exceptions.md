...
| `InvalidArgumentException` | `Resolver`, `PanelContext` | Missing or unknown `provider` config; a mistyped option in debug mode |
...
The model id the request was sent with.
::
...
try {
    ...
    $upstream = $error->getPrevious(); // The SDK's original exception
}
```
...
While building the Panel's context, Copilot also checks the shape of the options it normalizes for the Panel; `model`, `baseUrl`, and the provider passthrough keys go unchecked. These checks raise `InvalidArgumentException` **only when Kirby's `debug` option is on**, so that one typo can't take the whole Panel down. With debug off, a malformed option falls back to its default; a malformed entry inside `skills` or `promptTemplates` is skipped and the rest of the list is kept.

| Trigger                                   | Message                                                                    |
| ----------------------------------------- | -------------------------------------------------------------------------- |
| Option has the wrong type                 | `Invalid <path>: expected <types>, got <type>`                             |
| Option is outside its set of valid values | `Invalid <path>: <value>. Must be one of: <values>`                        |
| Provider keys differing only in case      | `Conflicting provider keys: <names> – provider names are case-insensitive` |
| Skill entry missing or malformed fields   | `Invalid skills[<key>]: <reason>`                                          |
| Two skills sharing the same `id`          | `Duplicate skill ids: <ids>`                                               |
| Prompt template with an empty field       | `Invalid promptTemplates[<key>]: <reason>`                                 |
