...
While building the Panel's context, Copilot also checks the shape of every option it reads. These checks raise `InvalidArgumentException` **only when Kirby's `debug` option is on**, so that one typo can't take the whole Panel down. With debug off, a malformed option falls back to its default, and a malformed entry inside a list option – a single skill or prompt template – is skipped while the rest of the list is kept.
