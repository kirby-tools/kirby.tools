...
::note
Skill IDs may contain letters, digits, hyphens, and underscores – anything else is rejected. Prefer lowercase by convention. `label` and `instructions` can each be localized independently. Missing translations fall back to English, then to the first defined language.
::
