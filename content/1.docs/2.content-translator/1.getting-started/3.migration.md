...
- DeepL requests for text whose only markup is `<cN/>` KirbyTag placeholders go out without HTML tag handling, which used to entity-escape the surrounding text (`s&#x27;ouvre`) and shift whitespace around placeholders. A [`tag_handling` you configured yourself](/docs/content-translator/php-classes/deepl-client) is respected unchanged.
