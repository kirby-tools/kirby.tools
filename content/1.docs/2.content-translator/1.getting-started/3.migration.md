...
- A custom [`Strategy`](/docs/content-translator/php-classes/strategies#implementing-a-custom-strategy) returns `null` for a unit it could not translate. The previously documented pattern – pre-filling the result with source text – still works but makes drops invisible to the new result; switch to `null` slots.
