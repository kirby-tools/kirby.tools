...
**Text segments kept their source text in {languages}**: the batch counterpart. A batch run judges every language on its own, so a language where nothing came back usable is named here rather than diluted into a total across languages.
...
Field key, target language, and the check that rejected it. The language code is part of the line because a batch run repeats every field once per language. A placeholder mismatch adds what the check expected and what it got – on every provider, since the route ships the indexes along with the reason. For the full list of reasons, see [Drop Reasons](/docs/content-translator/advanced/hooks#drop-reasons).
