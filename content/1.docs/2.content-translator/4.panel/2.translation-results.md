...
Field key, target language, and the check that rejected it. The language code is part of the line because a batch run repeats every field once per language. A placeholder mismatch adds what the check expected and what it got, on every provider. For the full list of reasons, see [Drop Reasons](/docs/content-translator/advanced/hooks#drop-reasons).
