---
title: Translation Coverage
description: Spot incomplete translations at a glance. The Languages view in the Panel gains a coverage dashboard and a list of pages that still need work.
---

The Languages view in the Kirby Panel gains a coverage dashboard and a pruned list of pages that still need translation work (Kirby 5 only). Open `/panel/languages` to see at a glance how complete each language is – and click straight through to the pages that are missing content.

::preview-dialog{src="/img/kirby-content-translator-translation-coverage.png" alt="Translation Coverage view in the Kirby Panel showing per-language completion rings and a tree of pages still missing translations" width="1280" height="800"}
::

## What You See

**Translation Coverage** – one ring per non-default language, showing the percentage of translatable fields that have content. The label below each ring reports how many pages are still incomplete (e.g. `4 pages incomplete`). A language at 100% is highlighted as positive.

**Pages to Translate** – a tree of pages that have at least one missing language. Branches collapse to keep the list compact: only incomplete pages and their ancestors are shown. Each entry links straight to its Panel view so you can fix it in one click.

If everything is translated, the tree is replaced with an empty state.

## How Coverage Is Calculated

Coverage uses the same translatable-field rules as the rest of the plugin. A field counts toward the totals if your blueprint marks it translatable (or it's translatable by default for its field type), and it counts as translated if its value in the secondary language is not empty.

::note
Default-language content is read without fallback when calculating coverage. A secondary-language content file that doesn't exist counts as zero translated fields, not as a copy of the default language.
::

The default language itself is excluded from the rings – it's the source, not a target.

## Caching

Coverage scans every page in the site, so the result is cached in Kirby's `pages` cache:

- **Per-page status** is computed lazily and cached on first read.
- **The aggregated tree index** (language stats and the pruned tree) has a 5-minute TTL.

The cache is automatically flushed whenever a page is saved, so editors see fresh numbers immediately after every change. The 5-minute TTL is just a safety net for saves that happen outside Kirby's normal commit lifecycle.
