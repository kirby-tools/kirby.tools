...
The PHP API includes error handling for KirbyTags to ensure that translation does not break content:

1. **Parse Errors**: If a KirbyTag cannot be parsed, it is wrapped in `<span translate="no">` to prevent breaking.
