...
The `copyContent`, `translateContent`, `translateTitle`, and `translateSlug` methods mutate the translator's internal model and don't return the updated instance. Use `model()` to retrieve the up-to-date model after calling them:
...
::note
`translateText()` returns the input unchanged for empty strings, pure numbers, and URLs.
::
