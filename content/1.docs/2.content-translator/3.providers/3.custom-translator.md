...
For per-unit failure handling, observability, and access to `TranslationUnit::$mode` / `$fieldKey`, implement the `Strategy` interface directly. See [Implementing a Custom Strategy](/docs/content-translator/php-classes/strategies#implementing-a-custom-strategy) for the full pattern – pre-fill source text, attempt each unit, emit a warning on per-unit failure, throw `TranslationException` only when zero units survived.
