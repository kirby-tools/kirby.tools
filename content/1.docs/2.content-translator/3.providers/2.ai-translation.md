...
The first toggle carries your configured backend – `DeepL`, or `Custom` when the `strategy` option names a closure or `Strategy` instance – and the second carries the Copilot provider in use (`ChatGPT`, `Gemini`, `Claude`, `Mistral AI`), falling back to `AI (Copilot)` when the Copilot context can't be read.
