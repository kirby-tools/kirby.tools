...
When both DeepL and Copilot are configured, you can choose your preferred strategy:
...
::note
Content Translator v3.12+ offers AI translation with Kirby Copilot v3.9.0 or later.
::
...
When you click the **{Language} → All Languages** button, a dialog appears letting you choose between DeepL and AI translation:

The first toggle carries your configured strategy – `DeepL`, or `Custom` when the `strategy` option names a closure or `Strategy` instance – and the second the name of the Copilot provider in use. Copilot is offered only when that provider has an API key.

::note
If only Copilot is configured (no DeepL), AI translation will be used automatically without showing the dialog.
::

::

::tip
The strategy selection dialog remembers your last choice across sessions, per browser.
::

## Custom System Prompt

The default system prompt protects content structure across providers. It instructs the AI model to preserve:
