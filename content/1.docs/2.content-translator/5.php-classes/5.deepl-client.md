...
The target language is resolved from the Kirby language code first, then from its `LC_ALL` locale, and matched against DeepL's [supported target languages](https://developers.deepl.com/docs/getting-started/supported-languages). A code that already names a regional variant wins over a locale that names a different one.
...
When neither names a supported target, `translateMany()` throws `LogicException`.
