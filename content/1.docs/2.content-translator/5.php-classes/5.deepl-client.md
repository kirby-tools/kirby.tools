...
::note
The client defaults to `tag_handling=html` and `split_sentences=1` to keep writer-field markup and markdown line breaks intact. `requestOptions` can override both, but `tag_handling` snaps back to `html` for texts carrying the plugin's `<span translate="no">` wrapper.
::
