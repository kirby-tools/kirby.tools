...
One class is the plugin's own, `TranslationException`; the other three are Kirby's. All extend `Kirby\Exception\Exception` and serialize cleanly to API responses. `TranslationException` fires only when zero units could be translated – partial failures emit a `:warning` hook instead.
