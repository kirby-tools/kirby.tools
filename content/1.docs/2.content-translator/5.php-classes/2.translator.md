...
One entry per rejected unit, each with its `fieldKey` – led by the container for a nested field, as in `body.text` – the `reason` it was rejected, and – on a `placeholder mismatch` – the `expectedIndexes` and `actualIndexes` lists.
::
