...
Translate raw strings without a model. Both methods return the source unchanged for text a provider would only corrupt: blanks, pure numbers, and standalone URLs. Those entries never leave your server – when a batch contains nothing else, no provider request is made at all.
