...
The return value reports what the run did – no hook wiring needed to notice a drop:
...
One entry per dropped unit, each with its `fieldKey`, the `reason` it was dropped, and – on a `placeholder mismatch` – the `expectedIndexes` and `actualIndexes` lists.
::
