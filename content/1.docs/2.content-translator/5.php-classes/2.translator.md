...
::tip
The same class is exposed as a model method on `Site`, `Page`, and `File`: `$page->translator()`. The `translator()` global helper mirrors the constructor.
::
...
The return value reports what the run did – no hook wiring needed to notice a rejection:

::field-group

::field{name="translatableCount" type="Int"}
How many units were handed to the strategy.
::

::field{name="translatedCount" type="Int"}
How many came back usable.
::

::field{name="rejections" type="TranslationRejection[]"}
One entry per rejected unit, carrying its `fieldKey`, the `reason` it was rejected, and on a `placeholder mismatch` the `expectedIndexes` and `actualIndexes` lists. A nested field's `fieldKey` leads with its container, as in `body.text`.
::

::

```php
$result = $translator->translateContent('de', 'de', 'en');

foreach ($result->rejections as $rejection) {
    // e.g. "text: placeholder mismatch"
    error_log("{$rejection->fieldKey}: {$rejection->reason}");
}
```
...
Translate the model title. Falls back to the source language when the target-language title is empty. A rejected translation leaves the title untouched – the source title is never written into the target language.
...
Translate the page's current slug and rename the page to the result. Independent of `translateTitle` – the slug string is what goes to the strategy, not the title. No-op for the homepage, the error page, non-page models, and a rejected translation.
...
Prefer `translateTexts()` over a loop; the built-in strategies batch.
