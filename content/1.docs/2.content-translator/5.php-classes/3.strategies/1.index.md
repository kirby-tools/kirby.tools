...
- **The result is validated afterwards.** Any unit that lost, invented or repeated a `<cN/>` token – and any slot that comes back missing, blank, or as a non-string – is reverted to source and reported through the warning hook, whichever strategy produced it. Returning an empty string does not blank the field; it counts as a failed unit.
