...
Similar to the `title` property, the model slug can be included in import and translation operations. The Panel builds the slug from the translated title, so `slug: true` also sends the title to the provider. The default language's slug never changes, since that would rename the page folder.
