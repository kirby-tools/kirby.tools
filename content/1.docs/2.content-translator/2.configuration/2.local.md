...
::note
This property covers the import actions only. Translation is confirmed by its own dialogs: batch translation always asks which languages to write, and per-language translation asks which provider to use whenever both DeepL and Copilot are configured.
::
