---
title: Global Configuration
description: Set the translation strategy, credentials, and project-wide defaults in config.php – applies to every view button and section.
---

## Translation Strategy

At least one translation strategy must be configured. Pick one below – the [`strategy` option](#strategy) selects which one runs.

::card-group

:::card{title="DeepL" icon="i-simple-icons-deepl" to="/docs/content-translator/providers/deepl"}
Industry-leading machine translation with a free tier available.
:::

:::card{title="AI Translation" icon="i-ri-sparkling-line" to="/docs/content-translator/providers/ai-translation"}
Context-aware translation via the [Kirby Copilot](/copilot) plugin (OpenAI, Anthropic, Google, Mistral).
:::

:::card{title="Custom Strategy" icon="i-ri-puzzle-line" to="/docs/content-translator/providers/custom-translator"}
Plug in any translation API via a closure or a custom `Strategy` implementation.
:::

::

## Available Properties

Set global defaults that apply to both **view buttons** and **sections**. These can be overridden in individual blueprints. The following configuration sets DeepL as the translation strategy and defines global defaults for field types, title translation, slug translation, confirmation dialogs, and KirbyTags translation:

```php [site/config/config.php]
return [
    'johannschopplich.content-translator' => [
        // Translation strategy
        'DeepL' => [
            'apiKey' => 'your-deepl-api-key'
        ],

        // Global default properties for view buttons and sections
        'fieldTypes' => [
            'blocks',
            'text',
            'textarea'
        ],
        'title' => true,
        'slug' => true,
        'kirbyTags' => [
            'link' => ['text', 'title'],
            'image' => ['alt', 'caption'],
            'file' => ['text', 'title']
        ]
    ]
];
```

Every view button and section property except `label`, `systemPrompt`, and `theme` can be set here as a project-wide default; the [View Button & Section Configuration](/docs/content-translator/configuration/local#available-properties) page describes each one. `systemPrompt` becomes [`ai.systemPrompt`](#aisystemprompt); `label` and `theme` stay in the blueprint. Everything below exists only globally.

### `strategy` :u-badge{label="Mixed" class="align-middle ml-2 rounded-full!" variant="subtle"}

Selects the translation strategy explicitly. Accepts a string preset, a closure, or a [`Strategy`](/docs/content-translator/php-classes/strategies) instance. Without this option the plugin uses the deprecated `translateFn` if one is configured, and DeepL otherwise.

| Value      | Resolves to                                                                                            | Panel label |
| ---------- | ------------------------------------------------------------------------------------------------------ | ----------- |
| `'deepl'`  | `DeepLStrategy` – requires `DeepL.apiKey`                                                              | `DeepL`     |
| `'ai'`     | `CopilotAIStrategy` – requires the [Kirby Copilot](/copilot) plugin                                    | no dialog   |
| `Closure`  | Wrapped in `CallableStrategy`. Signature: `fn (string $text, string $target, ?string $source): string` | `Custom`    |
| `Strategy` | Used as-is – instance of `JohannSchopplich\ContentTranslator\Translation\Strategy`                     | `Custom`    |

The Panel follows this option: a closure or `Strategy` instance enables the translation buttons without a `DeepL.apiKey`, and `'ai'` makes AI translation the only strategy on offer – with a single strategy there is nothing to pick, so no dialog renders.

::code-group

```php [String preset]
return [
    'johannschopplich.content-translator' => [
        'strategy' => 'deepl', // or 'ai'
    ]
];
```

```php [Closure]
return [
    'johannschopplich.content-translator' => [
        'strategy' => function (string $text, string $target, ?string $source): string {
            return myTranslateApi($text, $target, $source);
        },
    ]
];
```

```php [Custom Strategy]
use App\Translation\MyApiStrategy;

return [
    'johannschopplich.content-translator' => [
        'strategy' => new MyApiStrategy(),
    ]
];
```

::

::callout{icon="i-ri-arrow-right-line" color="info" to="/docs/content-translator/php-classes/strategies"}
See the **Strategies** reference for the full interface and built-in implementations.
::

::warning
The legacy `translateFn` option is deprecated and will be removed in v4. Migrate by renaming the key to `strategy` – the closure signature is identical.
::

### `batchConcurrency` :u-badge{label="Integer" class="align-middle ml-2 rounded-full!" variant="subtle"}

In batch translation mode, multiple languages are translated in parallel to improve performance. By default, up to 2 translations are processed concurrently to avoid hitting API rate limits.

Lower it to `1` when the provider returns rate limit errors – the languages then translate one after another:

```php [site/config/config.php]
return [
    'johannschopplich.content-translator' => [
        'batchConcurrency' => 1
    ]
];
```

### `ai.systemPrompt` :u-badge{label="String" class="align-middle ml-2 rounded-full!" variant="subtle"}

When using AI translation, you can replace the built-in system prompt with a custom one. Set it globally or override it per blueprint.

```php [site/config/config.php]
return [
    'johannschopplich.content-translator' => [
        'ai' => [
            'systemPrompt' => 'You are a medical translator. Preserve clinical terminology and abbreviations.'
        ]
    ]
];
```

::callout{icon="i-ri-sparkling-line" color="info" to="/docs/content-translator/providers/ai-translation#custom-system-prompt"}
See the **AI Translation** docs for the full default prompt, blueprint override examples, and usage details.
::

::callout{icon="i-ri-arrow-right-line" color="info" to="/docs/content-translator/configuration/local#configuration-precedence"}
For configuration precedence and blueprint overrides, see the **View Button & Section Configuration** docs.
::
