...
Placeholders are compared between the source and the translation once the strategy returns – which tokens came back, not how many. Any unit that lost, invented or repeated a `<cN/>` token is dropped back to its source text, and the [`content-translator.translate:warning`](/docs/content-translator/advanced/hooks#content-translatortranslatewarning) hook fires with the reason `placeholder mismatch`. Untranslated prose is the failure mode – never a mangled tag.

The check sits above the strategy, so it protects DeepL, AI, and any [custom `Strategy`](/docs/content-translator/php-classes/strategies) equally – a strategy cannot opt out of it.
