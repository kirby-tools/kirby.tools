...
Protection is structural, not prompt-based: tags are removed before translation and restored afterwards. The Panel and the [PHP API](/docs/content-translator/php-classes) produce the same result.
...
A translation that damages a tag is discarded, the field keeps its source text, and the [`content-translator.translate:warning`](/docs/content-translator/advanced/hooks#content-translatortranslatewarning) hook fires with the reason `placeholder mismatch`. Untranslated prose is the failure mode – never a mangled tag. This holds for DeepL, AI, and any [custom `Strategy`](/docs/content-translator/php-classes/strategies).
