...
::warning
Hooks live on the PHP side, and AI translation started in the Panel never gets there. It runs in the browser against Kirby Copilot, which owns the provider credentials, so none of the three hooks fire for it. Everything else does reach PHP: DeepL and custom-strategy translations go through the plugin's batch endpoint whether they start in the Panel or in your own code, and both `Translator` entry points run the full pipeline. Terminology enforcement, logging, or drop alerting wired up here therefore skips Panel AI translations silently.
::
...
::warning
Server-side translations only. AI translations started from the Panel run against Kirby Copilot in the browser and never reach PHP, so no warning fires for them – their reasons go to the browser console. DeepL and custom strategies from the Panel go through the batch endpoint and do fire.
::
...
::warning
A [custom strategy](/docs/content-translator/php-classes/strategies#implementing-a-custom-strategy) that returns `null` for a unit is recorded as a `missing translation` rejection, but **this hook does not fire for it** – the strategy owns that warning, being the only layer that knows the real reason. If you alert on drops, emit the hook from your own strategy.
::
