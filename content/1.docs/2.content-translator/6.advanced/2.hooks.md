...
::warning
A [custom strategy](/docs/content-translator/php-classes/strategies#implementing-a-custom-strategy) that returns `null` for a unit is recorded as a `missing translation` rejection, but **this hook does not fire for it**. A `null` slot means the strategy already knows the reason – a generic `missing translation` warning would report the drop a second time under a reason this layer had to guess. If you alert on drops, emit the hook from your own strategy.
::
