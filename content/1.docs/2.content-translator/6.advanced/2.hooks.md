...
::note
`:before` and `:after` fire for every unit holding content, including values no provider ever sees – a field that is just a price or a URL still runs both hooks, with the text unchanged. Branch on `$unit->fieldKey` or inspect `$text` if a hook should only act on prose.
::
