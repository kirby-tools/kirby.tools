---
title: Hooks
description: Preprocess or postprocess content with before/after hooks, and surface rejected translations with the warning hook.
---
...
The full `TranslationUnit` (`text`, `fieldKey`). Lets you branch on the originating field or table cell – but only where there is one, see [Field-Aware Preprocessing](#field-aware-preprocessing). A field inside a structure, object, blocks, or layout field leads with its container: `body.text`, not `text`.
::
...
::warning
A [custom strategy](/docs/content-translator/php-classes/strategies#implementing-a-custom-strategy) that returns `null` for a unit is recorded as a `missing translation` rejection, but **this hook does not fire for it** – the strategy owns that warning, being the only layer that knows the real reason. If you alert on rejections, emit the hook from your own strategy.
::
