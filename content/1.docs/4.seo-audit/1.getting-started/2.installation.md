...
### View Button :u-badge{label="Recommended" class="align-middle ml-2 !rounded-full" variant="subtle" color="success"}
...
To add the `seo-audit` view button to a particular view, set the `buttons` option in the corresponding blueprint. The button can be customized with the same configuration properties as the section:
...
See the [configuration guide](/docs/seo-audit/configuration/local) for all available options.

::note
The Panel view button requires Kirby 5. For Kirby 4, use the section below.
::

### Section :u-badge{label="Kirby 4 Compatible" class="align-middle ml-2 !rounded-full" variant="subtle"}
