...
description: Set the analysis to run on publish, the default log level, and the proxy that fetches pages from another origin.
---
...
### `analyzeOn` :u-badge{label="String" class="align-middle ml-2 rounded-full!" variant="subtle"}

Runs the analysis on its own after the editor publishes their changes, so the [rating](/docs/seo-audit/guide/ratings) keeps up with the content. Changing the status of a page does not count. The run is silent: no dialog opens and no notification appears. The dot and the rating update, and a section on the same view shows the new report. The only trigger is `publish`, and the option is off by default. A blueprint can turn it on or off per view button or section with the [`analyzeOn`](/docs/seo-audit/configuration/local#analyzeon) property.
...
### `urlResolver` :u-badge{label="Closure" class="align-middle ml-2 rounded-full!" variant="subtle"}
