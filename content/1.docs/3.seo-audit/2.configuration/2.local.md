...
Kirby SEO Audit can be added to Panel views via a **view button** (recommended) or a **section**, and both can be used together. The section adds the `label` and `persisted` properties and resolves Kirby queries in `keyphrase` and `synonyms`.
...
A static keyphrase works in both view button and section. Kirby queries (e.g., to use the page title automatically) are section-only:

```yaml
# Static value – works in view button or section
keyphrase: developers
```

```yaml
# Kirby query – section only
...
Controls how much the plugin logs to the browser console during analysis. Available levels are `error`, `warn` (default), `info`, and `debug`.
