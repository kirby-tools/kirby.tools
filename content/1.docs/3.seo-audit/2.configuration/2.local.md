...
**Default:** `positive`  
**Options:** Any theme Kirby's button component accepts, including the `*-icon` variants – see Kirby's [UI reference](https://getkirby.com/docs/reference/plugins/ui).
...
By default, SEO reports are saved to the browser's local storage, one per section, page, and language. This allows editors to close the Panel and return later without rerunning the analysis. It is also useful for comparing "before" and "after" results when optimizing content.
