...
Clicking the **SEO Audit** view button immediately runs an SEO and readability analysis for the current page and opens the results in a Panel dialog. The dot on the button is the page's [rating](/docs/seo-audit/guide/ratings).
...
Click the **Analyze** button to run an SEO and readability analysis of the current page. The report opens with one light per category, lists the results grouped into Good, OK, Needs improvement, Feedback, and Errors – empty groups are omitted – and closes with the analyzed version and the time:
...
- Use `keyphraseField` when editors should enter their own keyphrase per page.
- Use `keyphrase` for a static keyphrase or when pulling from a parent page via query.
...
::note
For every assessment the analysis can run, see [Assessments for SEO and Readability](/docs/seo-audit/guide/assessments).
::
...
### `analyzeOn` :u-badge{label="String | Boolean" class="align-middle ml-2 rounded-full!" variant="subtle"}

Runs the analysis on its own after the editor publishes their changes, without a dialog or notification, so the [rating](/docs/seo-audit/guide/ratings) stays current. The only trigger is `publish`. It can also be set [globally](/docs/seo-audit/configuration/global#analyzeon); a blueprint value overrides it, and `false` switches it off for one view button or section.
...
**Options:** Any theme Kirby's button component accepts, including the filled variants without the `-icon` suffix – see Kirby's [UI reference](https://getkirby.com/docs/reference/plugins/ui).
...
By default, SEO reports are saved to the browser's local storage, one per section, page, and language. This allows editors to close the Panel and return later without rerunning the analysis. The [rating](/docs/seo-audit/guide/ratings) the plugin keeps on the server is unaffected by this property.
