...
### `auto` :u-badge{label="String | Boolean" class="align-middle ml-2 rounded-full!" variant="subtle"}

Runs the analysis on its own after the editor publishes their changes, without a dialog or notification, so the [rating](/docs/seo-audit/guide/ratings) stays current. The only trigger is `publish`. It can also be set [globally](/docs/seo-audit/configuration/global#auto); a blueprint value overrides it, and `false` switches it off for one view button or section.
