...
The browser console always shows the URL the audit starts from, which is the preview URL before any `urlResolver` rewrite. To see the HTML extracted from the page that was actually fetched, which is the check for a rewrite, set [`logLevel`](/docs/seo-audit/configuration/local#loglevel) to `info`.
