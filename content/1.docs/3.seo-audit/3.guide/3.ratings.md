...
Once the page is published again after the analysis, the rating is stale. The dot then shows an exclamation mark in the color of the rating it no longer stands for. The section marks the same state below its report with "Content changed since". Run the analysis again, or let [`analyzeOn: publish`](/docs/seo-audit/configuration/global#analyzeon) do it after every publish.
