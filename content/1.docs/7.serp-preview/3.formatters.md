...
The page is the one being edited, which is what makes a formatter page-specific:
...
Each edit sends the current value to your site for formatting, so keep a formatter cheap. Reading the page's own content is fine; fetching a remote URL is not.
