...
The page is the one being edited, which is what makes a formatter page-specific. On the site view it is the home page; on a file view the formatter does not run and the snippet draws the value as it is:
...
Each change posts the current value to your site, at most once per 250 ms per line. The reply replaces the drawn text; a failed reply keeps the previous one, so a formatter that throws shows no error in the Panel and the failed request is only in the browser's network tab. A formatter that fetches a remote URL delays the snippet, not the typing.
