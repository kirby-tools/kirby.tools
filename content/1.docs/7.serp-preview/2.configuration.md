...
What the URL line starts with. Defaults to the URL Kirby runs under, so a staging install shows its own host until the production one is set here. The page's preview URL path follows, with its language prefix in a multi-language site; a blueprint with `preview: false` leaves the line at the site URL:
