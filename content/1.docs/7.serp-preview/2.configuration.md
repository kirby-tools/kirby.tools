...
The heading above the snippet. It defaults to the Panel language's own translation, which is `SERP Preview` in English, and accepts a translation key or a map of languages:
...
What the URL line starts with, before the page path. Defaults to the installation's own URL, which is what a production install already wants:
...
The field whose value becomes the title line. Name it as it appears in the blueprint; the section lowercases it to match Kirby's content keys:
...
Joined with spaces, so `About – Kirby Tools`.
