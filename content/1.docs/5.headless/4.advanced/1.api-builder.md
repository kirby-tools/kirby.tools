...
::warning
Order matters. The handler returns as soon as a middleware yields a `Response` or a `File`, so `hasBearerToken()` has to come first in your chain – otherwise a resolver hands out its result before the token is ever checked.
::
