...
Validates the bearer token against the configured `headless.token`. It does not redirect by default; pass `hasBearerToken(true)` to redirect to the Panel when no `Authorization` header is present and `headless.panel.redirect` is enabled:
