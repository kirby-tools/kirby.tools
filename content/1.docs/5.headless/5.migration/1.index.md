...
description: Upgrade from v3 to v9 with before/after examples for global route authentication, CORS config changes, and resolved blocks structure updates.
---

## Migrating From v8 to v9

**Breaking Change:** Kirby Headless v9 requires PHP 8.3, and the KQL endpoint resolves the request language the way Kirby's own API does.

### PHP 8.3 Is the Floor

The plugin now requires PHP 8.3 or newer. Kirby 5 itself still runs on 8.2, so an upgrade may mean moving the PHP version before the plugin.

### The Global Route Reads `X-Language`

A path without a language prefix used to be served in the default language whatever the request asked for. It now takes its language from the `X-Language` header, so a frontend that keeps its own routing can request `/about` in German. A prefixed path is unaffected and still wins over the header. See [Multi-Language](/docs/headless/usage/json-templates#multi-language).

This only applies with `headless.globalRoutes` enabled.

### `?language=` Wins Over `X-Language` in KQL

The KQL endpoint used to let the `X-Language` header override a `?language=` query parameter. Kirby's API reads the query parameter first, and the endpoint now follows it. Requests that send only one of the two are unaffected.

The query cache key follows the resolved language too, so cached answers from v8 no longer match and are rebuilt once.
