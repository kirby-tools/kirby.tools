...
- **Media URLs** such as `/media/pages/about/hero.jpg` never require the token, so images returned by `$file->url()` keep working in a browser that cannot send an `Authorization` header.
...
::note
`kql.auth` set to `'bearer'` without a `headless.token` falls back to Kirby's native API authentication for the `/api/kql` endpoint. Set `kql.auth` to `false` if you want the endpoint public on purpose.
::
