...
When enabled, a request that arrives without an `Authorization` header is redirected to the Panel login page – but only if it sends an `Accept` header asking for something other than JSON, which is what a browser navigation does. A client that asks for JSON is answered with JSON, and so is a client that sends no `Accept` header at all. Your frontend never has to follow a redirect it cannot use, and neither does `curl`.
