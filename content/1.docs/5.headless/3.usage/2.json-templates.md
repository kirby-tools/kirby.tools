...
Both `__template__` and `__sitemap__` answer from Kirby's pages cache when it is enabled. A request that is not a plain `GET`, or carries query or body data, is always rendered fresh. Send `X-Cacheable: false` to bypass the cache for a single request, the same header the [KQL endpoint](/docs/headless/usage/kql) accepts.
...
::note
All four filters apply – a page appears only if none of them rejects it.
::
