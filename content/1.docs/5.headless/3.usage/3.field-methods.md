...
Out of the box, one entry is configured: the `image` field of Kirby's `image` block – which is what the example above resolves. The `blocksResolver` option sits at the top level of `config.php`, not under `headless`. Configure it to reach other blocks:
...
The `toResolvedLayouts()` method extends Kirby's [`toLayouts()`](https://getkirby.com/docs/reference/templates/field-methods/to-layouts) method. The same block resolver configuration applies to the blocks in every column.
...
For headless setups, you may want to remove the origin or language prefix from URLs. The `permalinksResolver` option sits at the top level of `config.php`, like `blocksResolver`:
